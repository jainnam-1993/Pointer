...
void main() {
  ...
  group('AppSettings', () {
    ...
    test('copyWith creates new instance with updated values', () {
      const settings = AppSettings();
      final updated = settings.copyWith(hapticFeedback: false, autoAdvance: true, autoAdvanceDelay: 60, theme: 'light');
    ...
    test('toJson serializes correctly', () {
      const settings = AppSettings(hapticFeedback: false, autoAdvance: true, autoAdvanceDelay: 45, theme: 'cosmic');
    ...
    test('fromJson deserializes correctly', () {
      final json = {'hapticFeedback': false, 'autoAdvance': true, 'autoAdvanceDelay': 45, 'theme': 'cosmic'};
  ...
  group('StorageService - Onboarding', () {
    test('hasCompletedOnboarding returns false when not set', () {
      when(() => mockPrefs.getBool(StorageKeys.onboardingCompleted)).thenReturn(null);
      expect(storageService.hasCompletedOnboarding, false);
    });

    test('hasCompletedOnboarding returns stored value', () {
      when(() => mockPrefs.getBool(StorageKeys.onboardingCompleted)).thenReturn(true);
      expect(storageService.hasCompletedOnboarding, true);
    });

    test('setOnboardingCompleted saves value', () async {
      when(() => mockPrefs.setBool(StorageKeys.onboardingCompleted, true)).thenAnswer((_) async => true);

      await storageService.setOnboardingCompleted(true);

      verify(() => mockPrefs.setBool(StorageKeys.onboardingCompleted, true)).called(1);
    });
  });

  group('StorageService - Favorites', () {
    test('favorites returns empty list when not set', () {
      when(() => mockPrefs.getString(StorageKeys.favoritePointings)).thenReturn(null);
      expect(storageService.favorites, isEmpty);
    });

    test('favorites returns stored list', () {
      final stored = jsonEncode(['adv-1', 'zen-1', 'dir-1']);
      when(() => mockPrefs.getString(StorageKeys.favoritePointings)).thenReturn(stored);
    ...
    test('addFavorite adds new favorite', () async {
      when(() => mockPrefs.getString(StorageKeys.favoritePointings)).thenReturn(jsonEncode(['adv-1']));
    ...
    test('removeFavorite removes existing favorite', () async {
      when(() => mockPrefs.getString(StorageKeys.favoritePointings)).thenReturn(jsonEncode(['adv-1', 'zen-1']));
      when(() => mockPrefs.setString(any(), any())).thenAnswer((_) async => true);
  ...
  group('StorageService - Viewed Pointings', () {
    test('viewedPointings returns empty list when not set', () {
      when(() => mockPrefs.getString(StorageKeys.viewedPointings)).thenReturn(null);
    ...
    test('viewedPointings returns stored list', () {
      ...
      when(() => mockPrefs.getString(StorageKeys.viewedPointings)).thenReturn(stored);
    ...
    test('markPointingAsViewed adds new viewed pointing at front', () async {
      when(() => mockPrefs.getString(StorageKeys.viewedPointings)).thenReturn(
        jsonEncode([
          {'id': 'adv-1', 'viewedAt': 1234567890},
        ]),
      );
    ...
    test('markPointingAsViewed trims to 100 items', () async {
      final existing = List.generate(100, (i) => {'id': 'p-$i', 'viewedAt': 1234567890 - i});
      when(() => mockPrefs.getString(StorageKeys.viewedPointings)).thenReturn(jsonEncode(existing));
      when(() => mockPrefs.setString(any(), any())).thenAnswer((_) async => true);
  ...
  group('StorageService - Preferred Traditions', () {
    test('preferredTraditions returns empty list when not set', () {
      when(() => mockPrefs.getString(StorageKeys.preferredTraditions)).thenReturn(null);
    ...
    test('preferredTraditions returns stored list', () {
      ...
      when(() => mockPrefs.getString(StorageKeys.preferredTraditions)).thenReturn(stored);
