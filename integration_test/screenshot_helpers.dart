...
class ScreenshotCapture {
  ...
  Future<void> capture(WidgetTester tester, String stepName, {String? description}) async {
  ...
  String generateReport() {
    ...
    buffer.writeln('.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 20px; }');
    buffer.writeln('</style>');
    buffer.writeln('</head><body>');
    buffer.writeln('<h1>📱 Screenshot Report: ${ScreenshotConfig._currentFlow}</h1>');
    ...
    for (final record in records) {
      ...
      buffer.writeln('<img src="${record.fileName}.png" alt="${record.stepName}">');
...
class ScreenshotRecord {
  ...
  ScreenshotRecord({required this.fileName, required this.stepName, this.description, required this.timestamp});
}
