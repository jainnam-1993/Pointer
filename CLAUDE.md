...
**Key Deps**: Firebase Auth (Google/Apple Sign-In), in_app_purchase (tip jar), flutter_local_notifications + workmanager, just_audio, share_plus, screenshot
...
- **Subscription** (`subscription_providers.dart`): All features free (no IAP). Stub provides SubscriptionState with isPremium=true for API compatibility
...
- **Widget** (`widget_service.dart`): Pointings cache population with favorite interleaving, theme sync via refreshWidget()
...
**All Features Free**: No IAP or premium gating. All content (library, notifications, widget) is available to all users.
...
**Providers** (`providers/`): Settings (zen, OLED, accessibility, theme, auto-advance) | Content (round-robin persisted order, favorites, affinity) | Subscription (all features free, stub isPremium=true) | Donation (tip jar, in_app_purchase consumables, DonationState)
...
- `/docs/PRIVACY_POLICY.md` - Privacy policy (local-first, effective Jan 2 2025)
