...
/// Keys use the suffix (without package prefix) for production products.
const _tipProducts = <String, _TipProduct>{
  ...
  // Android test product
  'android.test.purchased': _TipProduct('Test Tip', Icons.bug_report_outlined),
};

/// Extract the product suffix from a full product ID
/// e.g., 'com.dailypointer.tip_small' -> 'tip_small'
String _getProductKey(String productId) {
  // Handle Android test product ID
  if (productId == 'android.test.purchased') {
    return productId;
  }
  // Extract suffix after last dot for production IDs
  final lastDot = productId.lastIndexOf('.');
  if (lastDot != -1 && lastDot < productId.length - 1) {
    return productId.substring(lastDot + 1);
  }
  return productId;
}
  ...
  Widget _buildProductsGrid(List<ProductDetails> products, PointerColors colors) {
    ...
    return GridView.count(
      ...
      children: products.map((product) {
        final productKey = _getProductKey(product.id);
        final config = _tipProducts[productKey];
